...
const { i18nMenus, i18nCommon, i18nPermissions } = useMessage()
...
const targetItems = computed(() => MENU_TARGET.items.map(({ value, label }) => ({ label: i18nMenus(label), value })))
...
<template>
  <AutoFormModal
    ...
    :title="i18nMenus(data?.id ? 'edit' : 'add')"
